...
export default function ContentViewPage() {
  ...
  const fetchSavedProgress = async () => {
    try {
      const response = await fetch(`/api/progress?contentId=${params?.id}`);
      if (!response.ok) {
        const errorText = await response.text();
        console.error('API Error:', response.status, errorText);
        throw new Error(`HTTP error ${response.status}`);
      ...
      if (data.success && data.data && data.data.length > 0) {
        const progressItem = data.data[0];
        setSavedPosition(progressItem.progress || 0);
      }
    } catch (error) {
  ...
  const handleProgress = async (progress: number, currentTime?: number) => {
    ...
    if (Math.abs(progress - lastProgressUpdate) < 5) return;
  ...
  const video_url = "https://www.youtube.com/watch?v=HBCwzfFbopE&pp=ugUHEgVlbi1VU9IHCQkjCgGHKiGM7w%3D%3D";
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b sticky top-0 z-50 shadow-sm">
        <div className="container mx-auto px-4 py-3">
          <Button
            variant="ghost"
            onClick={() => router.push('/customer/content')}
              ...
              {content.type === 'video' ? (
                <VideoPlayer               
                  ...
                  onError={(error) => {
                    ...
                    console.error('Original URL:', content.fileUrl);
                    console.error('Processed URL:', processVideoUrl(content.fileUrl));
              ...
              <div className="space-y-3">
                {recommendations.map((item) => (
                  <Card
                    ...
                    onClick={() => router.push(`/customer/content/${item._id}`)}
                  >
